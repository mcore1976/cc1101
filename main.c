 ...
 * based on RFBOOT by code Panagiotis Karagiannis
...
static volatile uint8_t ccreceivebuffer[CC1101_BUFFER_LEN] = "123456789012345678901234567890123456789012345678901234567890123456789012345678901234\x00";

// buffer for RX UART data to further send over CC1101
static volatile uint8_t uartreceivebuffer[BUFFER_SIZE] = "123456789012345678901234567890123456789012345678901234567890123456789012345678901234\x00";
...
static volatile uint8_t receivebufferpos = 0;
...
#define CC1101_TXuint8_tS           0x3A        // Underflow and Number of uint8_ts
#define CC1101_RXuint8_tS           0x3B        // Overflow and Number of uint8_ts
...
bool cc1101_sendData(CCPACKET packet)
{
  ...
  if((readStatusReg(CC1101_TXuint8_tS) & 0x7F) == 0)
    res = true;
...
{
  uint8_t val;
  uint8_t rxuint8_ts = readStatusReg(CC1101_RXuint8_tS);
...
// initialization of Serial UART with predefinded speed
...
// send single character, no interrupt and buffer in use
...
// WAIT and RECEIVE single character from serial buffer, no interrupt in use
...
// send string of characters from normal memory
void USART_putstring(char s[])
{
 int i = 0;
	while ( (s[i] != 0x00) & (i<(strlen(s))) )
	{
                if (s[i]==0x0d)  USART_send(0x0a);
		USART_send(s[i]);
		i++;
	};
}

// send string of characters from receivebuffer with CRLF

void USART_putstringmod(char s[])
{
 int i = 0;
	while ( i<receivebufferpos )
	{       // send CRLF instead of only CR  
                if (s[i]==0x0d)  USART_send(0x0a); 
		USART_send(s[i]);
		i++;
	};
...
// ************************************************************************ 
// INTERRUPT ROUTINE FOR UART HANDLING RECEIVING CHARS
// will put received char into  BUFFER and set the flag
// ************************************************************************ 
...
ISR(USART_RX_vect)
{
        ...
        while ( !(UCSR0A & (1<<RXC0)) );
	if (receivebufferpos==BUFFER_SIZE)                 // if BUFFER_SIZE is reached, reset to start of buffer.
		receivebufferpos=0;                        // set pointer to buffer beginning  
	uartreceivebuffer[receivebufferpos++] = UDR0;      // increment rxn and return new value.   
	uartdatareceived=true;                             // notify main code of receipt of data.
	sei();                                             // enable interrupt back
}
...
ISR (INT0_vect)
{
 ...
 // Disable interrupt first
  cli();
    ...
    {
        // data received so we blink the LED   
        PORTC |= _BV(PC5);   // Toggles ON the LED pin
        ...
        ccreceivebuffer[inpacket.length+1] = 0x00; 
...
int i;
  ...
  // FIRST WE BLINK ON SEVERAL TIMES TO INDICATE PROGRAM IS WORKING
...
while(1)
 { // neverendingloop for receiving / transmitting

    // wait for data received over CC1101
    if ( (data_ready == true) && (data_reception == true)) 
       {
    // send received data over serial port simple, no interrupts
        USART_putstring(ccreceivebuffer);
    // LED change       
        PORTC &= ~_BV(PC5);  // Toggle LED OFF
    // something was received, we have to celar the status
        data_ready = false;    // END of IF statement  
        data_reception = false;
        };

    // wait for data transmitted over CC1101, switch off LED, clear flags
    if ( uartdatareceived == true)
        {  
        // LED change       
        PORTC &= ~_BV(PC5);  // Toggle LED OFF
        // copy text from UART RX buffer and send it over CC1101
        memcpy(tosend, uartreceivebuffer, receivebufferpos); 

        // send received data over serial port back to serial port (ECHO)
        USART_putstringmod(tosend);

        // send received data over serial to radio over CC1101 
        radiosendtxt(tosend);

        // clear all flags   
	uartdatareceived = false;       // clear the uart RX buffer flag
	receivebufferpos = 0;           // set the position to zero
        data_ready = false;  
