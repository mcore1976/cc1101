...
#define SPI_SS   B,3     // PB3 = SPI_SS
#define SPI_MOSI B,1     // PB0 = MOSI
#define SPI_MISO B,0     // PB1 = MISO
#define SPI_SCK  B,2     // PB2 = SCK
#define GDO0     B,4     // PB4 = PCINT4 pin
...
void spi_init() 
{
  ...
  //digitalWrite(SPI_SS, HIGH); 
     ...
     PORTB &= ~_BV(1);  // MOSI
  // PORTB &= ~_BV(2);  // set SCK to LOW for CC1101 communication
  // PORTB |=  _BV(3);   // SPI_SS 
     PORTB |= _BV(2);   // SCK
    
  // USICR=(1<<USIWM0);                //Choosing SPI aka three wire mode on USI
...
{
PORTB &= ~_BV(3);      // SS - PORTB3 of ATTINY85
};
...
void cc1101_wakeUp(void)
{
  ...
  PORTB |= _BV(2);                      // set SCK to HIGH again
...
ISR (PCINT4_vect)
{
    ...
    // update again PCINT4 conditions
...
void radio_init(void) {
    ...
    /*
    GIMSK |= (1 << PCIE);   // pin change interrupt enable
    PCMSK |= (1 << PCINT4); // pin change interrupt enabled for PCINT4 -GD0 output from CC1101ssss
    */
...
void radiosendtxt(char *send_str)
 {
   ...
   // wait for PCINT4 falling edge on GCO0 
   // while (! data_ready);
...
// 500ms at 8.0 MHz
   ...
   "    nop	\n"
    ...
    // setup interrpupts for GD0 connected to INT0
    //
    DDRB &= ~_BV(4);         // GD0 connected to PB4 - switch as input
    PORTB |= (1 << PB4);     // enable pullup on GD0 and PCINT4 interrupt

    /*
    GIMSK |= (1 << PCIE);   // pin change interrupt enable
    PCMSK |= (1 << PCINT4); // pin change interrupt enabled for PCINT4 - GD0 output from CC1101
    sei();                  // enable interrupts of ATTINY85
    */
     
    // and now sending data over CC1101, no receiving functions will be used here
...
while(1)
 {       // neverendingloop for transmitting
       ...
       //    memcpy(tosend, uartreceivebuffer, uartreceivebufferpos);
        memcpy(tosend, "This is just a test of ATTINY85 and CC1101", 42);
