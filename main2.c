 ...
 * This version is tuned up to external XTAL 3.6864 MHz
 * which gives 0% error rate to all serial port speeds up to 115200baud 
 *
 * (c) Adam Loboda 2018
 * 
 */
...
// Calculate UART register setup for XTAL 3.6864 MHz
...
#define LED_PORT PORTC                                         // signalling LED for RX & TX over RS232/Radio
...
{
        while ( !(UCSR0A & (1<<RXC0)) );
	if (uartreceivebufferpos==BUFFER_SIZE)                 // if BUFFER_SIZE is reached, reset to start of buffer.
		uartreceivebufferpos=0;                        // set pointer to buffer beginning  
	uartreceivebuffer[uartreceivebufferpos++] = UDR0;      // increment rxn and return new value.   
...
while(1)
 { // neverendingloop for receiving / transmitting
    ...
    if ( (data_ready == true) && (data_reception == true)) 
       {
     // copy text from CC RX buffer and send it over serial port
        memcpy(tosend, ccreceivebuffer, ccreceivebufferpos);

    // put NULL at the end
        tosend[ccreceivebufferpos] = 0x00;

    // send received data over serial port simple, no interrupts
        USART_putstring(tosend);


    // LED change       
        PORTC &= ~_BV(PC5);  // Toggle LED OFF

    // something was received, we have to celar the status - especially CC receive buffer position
    ...
    if ( uartdatareceived == true)
        ...
        memcpy(tosend, uartreceivebuffer, uartreceivebufferpos);

        // put NULL at the end of this string
        tosend[uartreceivebufferpos] = 0x00; 
